...
(function() {
    ...
    DrawingManager.prototype._calculate = function(overlay, point) {
        ...
        if (this._enableCalculate && BMapLib.GeoUtils) {
            ...
            if (result.data)
                result.label = this._addLabel(center, result.data);
            //result.label = this._addLabel(center, formatCalculateResult(result.data,isArea));
        }
        return result;
    }
    ...
    DrawingManager.prototype._addGeoUtilsLibrary = function() {
        if (!BMapLib.GeoUtils) {
            ...
            script.setAttribute("src", 'lib/geoutils.min.js');
